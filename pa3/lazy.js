...
function map(f, thunk_xs) {
  return delay(function () {
    const current = force(thunk_xs);
    return {
      head: f(current.head),
      tail: map(f, current.tail)
    };
  });
}
...
function zipWith(f, thunk_xs, thunk_ys) {
  return delay(function () {
    const current1 = force(thunk_xs)
    const current2 = force(thunk_ys)
    return {
      head: f(current1.head, current2.head),
      tail: zipWith(f, current1.tail, current2.tail)
    };
  });
}
...
function tail(thunk_xs) {
  return delay(function () {
    const current = force(thunk_xs)
    const next = force(current.tail)
    return {
      head: next.head,
      tail: tail(current.tail)
    };
  });
}
